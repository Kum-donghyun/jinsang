const RESULT_TYPES = {
    FACT_BOMBER: {
        ...
        img: "result_fact.png",
        desc: [
            "💡 <strong>나의 알바 성향은?</strong><br>감정에 쉽게 휘둘리지 않고 매뉴얼과 규정을 칼같이 지키는 냉철한 원칙주의자입니다. 억지 부리는 진상 손님을 마주해도 당황하지 않고, 조목조목 팩트를 짚어내며 이성적으로 대처하는 능력이 탁월합니다. 매장 입장에서는 가장 신뢰할 수 있는 든든한 해결사 타입입니다.",
...
let currentStep = 0;
const scoreBoard = { FACT_BOMBER: 0, ANGEL: 0, SWEET_BITTER: 0, MAD_DOG: 0, DEALER: 0, ROBOT: 0 };
...
function renderQuestion() {
    ...
    document.getElementById("question-number").innerText = `Q${currentStep + 1} / ${quizData.length}`;
...
function showResult() {
    ...
    let maxType = "";
    let maxScore = -1;
    for (let type in scoreBoard) {
        if (scoreBoard[type] > maxScore) {
            maxScore = scoreBoard[type];
            maxType = type;
        }
    }
    
    const result = RESULT_TYPES[maxType];
    document.getElementById("result-subtitle").innerText = result.subtitle;
    document.getElementById("result-title").innerText = result.title;
    
    // 📸 이미지 태그를 찾아 고유 파일명을 다이렉트로 매핑하는 안전한 로직
    const resultImage = document.getElementById("result-character-image");
    if (resultImage) {
        resultImage.src = result.img;
        resultImage.alt = result.title;
    }
    
    const descContainer = document.getElementById("result-desc-container");
    descContainer.innerHTML = "";
    result.desc.forEach(text => {
